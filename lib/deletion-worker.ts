...
export async function runDeletionJob(remote:DeletionProvider=provider,now=Date.now()) {
  ...
  try {
    ...
    if(job.db_step!=="completed") await db.transaction(async tx=>{
      ...
      for(const table of ["attempts","objective_states","lesson_activities","study_sessions","activities","learning_objectives","patches","retention_state","chat_messages","review_logs","daily_review_plans","cards","card_sets","sources","folders","user_profiles","consent_events","user_consents","ai_operations","deletion_challenges"]) {
