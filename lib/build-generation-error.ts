export function buildGenerationError(code: string, kind = ''): string {
  ...
  if (code === 'AI_REQUEST_FINAL') return 'Your previous generation has been resolved. Retry to start a new generation.';
  if (code === 'AI_PROVIDER_FAILED') return 'The AI service could not complete generation. Your material is still here. Retry to start a new generation.';
