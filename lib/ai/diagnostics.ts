import { mcqValidationCodes } from './mcq-contract.ts';
...
const codes = new Set(['rate_limit_exceeded', 'insufficient_quota', 'invalid_api_key', 'invalid_request_error', 'server_error', 'model_not_found', 'context_length_exceeded', 'incomplete', 'refusal', 'empty_output', 'invalid_json', 'invalid_cards', 'invalid_choices', 'invalid_material']);
const stages = new Set(['mcq_choices', 'material', 'response_json']);
const validationCodes = new Set([...mcqValidationCodes, 'malformed_json', 'cards_shape', 'material_shape']);
export function logAiDiagnostic(fields: Record<string, unknown>, sink: (line: string) => void = console.info) {
  const out: Record<string, string | number | boolean> = { event: 'ai_diagnostic', timestamp: new Date().toISOString() };
  ...
  for (const [key, values] of [['category', categories], ['reason', reasons], ['providerCode', codes], ['validationStage', stages], ['validationCode', validationCodes]] as const) {
  ...
  for (const key of ['itemIndex', 'choiceCount', 'duplicateCount', 'emptyChoiceCount', 'nonStringChoiceCount']) {
    const value = fields[key];
    if (typeof value === 'number' && Number.isSafeInteger(value) && value >= 0 && value <= 10000) out[key] = value;
  }
  if (typeof fields.correctIndexValid === 'boolean') out.correctIndexValid = fields.correctIndexValid;
