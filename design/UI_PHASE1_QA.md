...
## Final reading artwork integration — baseline `ac0f6d1`, 2026-09-14

Supplied `~/Downloads/patch-2.png` (final modification 21:56 JST) is the seated green-book mascot with genuine RGBA transparency, 1254×1254. Alpha decoding confirmed transparent corners and values spanning 0–255. Copied byte-for-byte to `public/patch/mascot-reading.png`, preserving the Downloads original; source, repository and built mobile asset SHA-256 all equal `055aedb6aad662578cc33b96fe8df214398d838a2372a98e9917ceeff4499331`. No image generation, background processing or cropping was used.

The previous temporary reading crop is replaced. `app/mascot.tsx` already maps the reading pose to this final filename, so no UI code/CSS changes or presentation adjustments were needed. Affected consumers are `app/home-screen.tsx` (active/hot/broken/completed Home) and `app/lesson-preview.tsx` (My Lesson). All three assets are now **RESOLVED** in `ASSET_REQUESTS.md`; prior pending statements above describe earlier checkpoints.

Actual rendered screenshots are in `/private/tmp/patch-reading-final-qa/`. Visually reviewed Active Home and My Lesson at 393×852, completed/post-lesson Home at 393×852, Active Home at 320×568 and 430×852, and the long Japanese preview at 320×568. The reading art has clean transparent surroundings, no visible white/paper edge, intact book/feet and ground shadow, appropriate scale and unchanged surrounding spacing. Existing browser assertions also passed for 768px reflow, replacement-image geometry, preview controls and completion-to-Home.

Validation using Node 22.23.2 and `PATCH_ENV=development`: `npm run check` PASS — typecheck, lint (0 errors / 39 existing warnings), **229/229 tests**, guarded Web build and artifact seal. `npm run mobile:build:local` PASS, including artifact seal and byte-identical packaged PNG (existing bundle-size warning). `npm run test:ui-browser` PASS for all seven states at 320/393/430/768 widths, interaction and stable-layout checks. No additional tests were introduced for this asset-only replacement. Swift/Simulator checks were not rerun because native code/Widget resources are unaffected. No API, backend, domain, DB/schema/migration changes. No merge or deployment.

## UI + U2 integration checkpoint — 2026-09-14

UI Phase 1 `076a35c` is preserved on `codex/patch-integration-checkpoint`, with U2 `f4588a2`. Assigned Lessons use existing Home action-card styling and the same preview; the original Continue priority remains intact. U2 completion reuses mascot/results/streak presentation with an activity-specific label, then returns to the current authoritative Home. No mascot files or original reference images changed.

Existing seven-state UI regression passes at 320/393/430/768 widths. Full App captures in `outputs/integration/` cover Home, preview, five Activity kinds, AI Help, completion and both unqualified/qualified post-Home at 320×568, 393×852 and 430×852. Final results, reproduction and known boundaries: `docs/patch-integration-checkpoint.md`.
