...
export function DomainLesson({ lessonId, onHome }: { lessonId: string; onHome: () => void }) {
  ...
  return <LessonExperience key={`${account.scope.account.userId}:${account.scope.account.sessionId}:${lessonId}`} sessionKey={adapterSession} adapter={adapter} checkpoint={checkpoint} onHome={onHome} />;
}
