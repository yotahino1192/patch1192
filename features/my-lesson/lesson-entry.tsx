...
import type { LessonViewModel } from './contracts';

/** A selected, server-issued Lesson ID. Home/Continue selection policy remains unchanged. */
export function LessonEntry({ children, onHome, renderComplete }: { children: ReactNode; onHome?: () => void; renderComplete?: (lesson: LessonViewModel, actualSeconds: number, onHome: () => void) => ReactNode }) {
  ...
  return <DomainLesson lessonId={selection.id} onHome={home} renderComplete={renderComplete ? (lesson, seconds) => renderComplete(lesson, seconds, home) : undefined} />;
}
