...
try {
 ...
 await cdp('Runtime.enable');await cdp('Page.enable');await cdp('Page.navigate',{url:'http://127.0.0.1:5228/tests/fixtures/domain-lesson.html'});
 ...
 await evaluate('Array.from(document.querySelectorAll("button")).find(b=>b.textContent==="わからない・AIに聞く").click()');await primary();await until(async()=>(await text()).includes('プレビューの説明'));await evaluate('Array.from(document.querySelectorAll("button")).find(b=>b.textContent==="閉じる").click()');assert.equal((await query('attempts',activities[0].id)).length,0);
 ...
 await primary();await until(()=>evaluate('!!document.querySelector("input[type=radio]")'));await choose();await evaluate('lessonFixture.slow=true;for(let i=0;i<12;i++)document.querySelector("[data-primary]").click()');await until(async()=>(await state())==='FEEDBACK');assert.equal((await query('attempts',activities[1].id)).length,1);
 ...
 await answer();await evaluate('lessonFixture.fail=true');await primary();await until(async()=>(await state())==='ERROR');assert.equal((await query('attempts',activities[3].id)).length,0);assert.equal((await text()).includes('private upstream'),false);
 ...
 console.log('PASS: signed A -> real Patch/Objective/six Activities/Lesson -> durable Attempt -> progress -> server Complete -> UI; no Help writes; lost-response retry, duplicate tap, API failure, reload/resume, B isolation, stale switch/logout, AbortSignal');
} finally {
