...
import { patchDuePresentation } from "../lib/patch-due-presentation";
...
export function SetLibrary({ data, now, folderId, openSetId, onFolder, onSet, onStudy, onData, onAdd, children }: {
  ...
  const folderCards = (items: Folder[]) => <div className="folder-grid">{items.map((item) => <button className="folder-tile" key={item.id} onClick={() => navigate(item.id)}><span className="folder-icon" aria-hidden="true"><svg width="28" height="28" viewBox="0 0 32 32"><path fill="currentColor" opacity=".7" d="M3 7a3 3 0 0 1 3-3h7l4 5h10a2 2 0 0 1 2 2v15H3Z"/><rect fill="currentColor" x="3" y="10" width="26" height="18" rx="3"/></svg></span><strong>{item.name}</strong><small>{t("Patch：{0}件", data.sets.filter((set) => set.folderId === item.id).length)}</small><span className="folder-arrow"><PatchIcon name="chevron" size={19} /></span></button>)}</div>;
  const patchRows = (items: AppData["sets"]) => <div className="library-sets">{items.map((set) => <article key={set.id}><button className="library-set-open" onClick={() => onSet(set.id)}><span className="library-patch-icon" aria-hidden="true">{setEmoji(set)}</span><div><strong>{set.title}</strong><small>{t("{0}枚", activeCards(set).length)}{set.category ? <> · {set.category}</> : null}</small></div><PatchIcon name="chevron" size={20} /></button></article>)}</div>;
  return <>
    <div className={`page library-page patch-library ${root && !currentSet ? "patch-library-root" : ""}`}>
      ...
      {searching ? <section className="material-search-results" aria-label={t("検索結果")}>
        ...
        <section className="patch-library-section today-patches" aria-labelledby="today-patches-heading">
          ...
          {visibleDuePatches.length > 0 ? <div className="today-patch-list">{visibleDuePatches.map(({ set, cards }) => { const due = patchDuePresentation(cards, retention, now.getTime()); return <button key={set.id} type="button" onClick={() => onStudy(`__daily__:${set.id}`)}><span className={`today-patch-icon ${due}`} aria-hidden="true">{setEmoji(set)}</span><span><strong>{set.title}</strong><small>{t("{0}枚の復習", cards.length)}</small></span><span className={`review-ready-chip ${due}`}>{due === "overdue" ? <svg width="13" height="13" viewBox="0 0 16 16" aria-hidden="true"><circle cx="8" cy="8" r="7" fill="currentColor"/><path d="M8 4v5m0 2v.2" stroke="white" strokeWidth="1.5" strokeLinecap="round"/></svg> : <PatchIcon name="calendar" size={13} />}{t(due === "overdue" ? "期限超過" : due === "today" ? "本日期限" : "復習可能")}</span><PatchIcon name="chevron" size={20} /></button>; })}</div> : !stale && <div className={`today-patch-empty ${completed ? "is-complete" : ""}`}><PatchIcon name={completed ? "check" : "calendar"} size={28} /><strong>{t(completed ? "今日の目標は完了です" : "復習はありません")}</strong><span>{t(completed ? "さらに学ぶ場合は、下のPatchから選べます。" : "次の復習まで、保存したPatchを自由に学べます。")}</span></div>}
