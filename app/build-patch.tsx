  ...
  return <div className={`build-flow import-page build-step-${step}`} lang="en">
    ...
    {step === 2 && <section className="build-content build-material">
      ...
      {!!importDraft.attachments.length && <ul className="build-files">{importDraft.attachments.map(file => <li key={file.id}>
        <PatchIcon name="document" size={30} /><div><strong>{file.name}</strong><small>{file.size !== undefined ? file.size < 1024 ? `${file.size} B` : file.size < 1024 * 1024 ? `${(file.size / 1024).toFixed(1)} KB` : `${(file.size / 1024 / 1024).toFixed(1)} MB` : `${file.text.length.toLocaleString()} characters`}</small><span className="sr-only">{file.status === 'reading' ? 'Checking file' : file.status === 'failed' ? 'File not accepted' : 'Accepted for processing'}</span></div>
