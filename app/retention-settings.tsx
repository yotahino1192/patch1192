...
export function RetentionSettings(){
 ...
 return <section><h3>{t("通知")}</h3><label><input type="checkbox" checked={s.reviewReminder} onChange={e=>set({...s,reviewReminder:e.target.checked})}/> {t("復習リマインダー")}</label><input aria-label={t("復習通知の時刻")} type="time" value={s.reminderTime} onChange={e=>set({...s,reminderTime:e.target.value})}/><label><input type="checkbox" checked={s.streakWarning} onChange={e=>set({...s,streakWarning:e.target.checked})}/>{t("連続学習のリマインダー（期限3時間前）")}</label><p>{t("通知を有効にして保存すると、iOSで通知の許可を確認します。通知はこの端末で同期できた当日分のみです。")}</p><button onClick={()=>void save()}>{t("通知設定を保存")}</button><p role="status">{t(message)}</p></section>;
}
