...
export function LessonCompletion({ data, now, cards, mistakes, unit = "cards" }: { data: AppData; now: Date; cards: number; mistakes: number; unit?: "cards" | "activities" }) {
  const { t } = useLanguage();
  ...
  return <>
    ...
    <h1 id="lesson-complete" tabIndex={-1}>{t("レッスンが終了しました")}</h1>
    <p className="patch-success-message">{name ? t("よくできました、{0}！", name) : t("よくできました！")}</p>
    <div className="patch-results"><div><span><PatchIcon name="book" size={30} /></span><strong>{cards}</strong><small>{t(unit === "activities" ? "完了したアクティビティ" : "完了したカード")}</small></div><div><span><PatchIcon name="refresh" size={30} /></span><strong>{mistakes}</strong><small>{t("もう一度の回数")}</small></div></div>
