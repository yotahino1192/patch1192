...
async function handleGET(request:Request) {try{return json(await deletionStatus(request.headers.get("x-deletion-receipt")||""));}catch(e){return authErrorResponse(e)||(e instanceof InputError ? Response.json({error:e.message},{status:e.status}) : null)||json({error:"Unavailable"},503);}}
