import { validGeneratedCards } from '../../../lib/material-validation';
...
const validCards = validGeneratedCards;
...
async function handlePOST(request: Request): Promise<Response> {
  try {
    ...
    if (body.action === "saveSet") {
      ...
      if (!material || (material.sourceKind !== undefined && !["source", "topic"].includes(material.sourceKind)) || !boundedText(material.title, 120) || !boundedText(material.sourceContent, 30000) || !boundedText(material.category, 60, true) || !boundedText(material.summary, 10000, true) || !Array.isArray(material.keyPoints) || material.keyPoints.length > 100 || !material.keyPoints.every((point) => boundedText(point, 5000)) || !validCards(material.cards)) {
    ...
    if (body.action === "addCardsToSet") {
      ...
      const cardIds = await addCardsToSet(userId, setId, body.cards, typeof body.sourceContent === "string" ? { title: String(body.sourceTitle || "追加資料").slice(0, 120), content: body.sourceContent.trim(), kind: body.sourceKind === "topic" ? "topic" : "source" } : undefined, typeof body.operationId === 'string' ? body.operationId : undefined);
    ...
    if (body.action === "undoReview") {
      ...
      return json({ data: await loadAppData(userId, [sessionId]) });
    }
    if (body.action === "reviewCard") {
      const cardId = String(body.cardId || "");
      const rating = String(body.rating || "") as BinaryReviewRating;
      if (!cardId || (body.selectedChoice === undefined && !["again", "good"].includes(rating))) {
      ...
      if (body.selectedChoice !== undefined && !boundedText(body.selectedChoice, 10000)) return json({error:'選択肢を確認してください。'},400);
      if (body.contentRevision !== undefined && (typeof body.contentRevision !== 'string' || !/^[a-f0-9]{64}$/.test(body.contentRevision))) return json({error:'教材を再読み込みしてください。'},400);
      const sessionId = body.sessionId;
      const reviewId = await reviewCard(userId, cardId, rating, body.responseMs, sessionId, { operationId: body.operationId, expectedReviewCount: Number(body.expectedReviewCount), selectedChoice: body.selectedChoice as string | undefined, contentRevision: body.contentRevision as string | undefined, requireSession: true });
