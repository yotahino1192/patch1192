...
### AI Hardening integration

AI requests now use the mandatory durable gateway in `lib/ai/control.ts`: current consent/lifecycle before input/ownership checks and reservation, revalidation immediately before dispatch and before atomic result/usage persistence. Cached replay checks the original consent revision. The deletion worker clears AI result JSON and request fingerprints while retaining minimal ledger state/cost references under the existing lifecycle tombstone; deletion cannot reset the shared AI budget or silently refund unknown calls. See `docs/production-hardening.md` for operator reconciliation and disaster-recovery limits. Privacy SQL migration 0008 is immutable; AI schema additions use 0009.
