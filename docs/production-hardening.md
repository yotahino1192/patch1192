...
- `failed_final`: known provider rejection (including 429 and completed non-success response), completed material parse/validation failure, or explicit operator resolution; maximum charge retained conservatively, concurrency released. Same key cannot dispatch again. Development-only abandonment below does not assert a known provider outcome.
...
For an unresolved operation belonging to the requesting user, a **different** key now receives `AI_PREVIOUS_UNRESOLVED` (429), while the original key continues to receive `AI_UNKNOWN`. Both paths reject before reservation/dispatch. Add Material explains that an administrator must resolve the operation; Retry never automatically replaces an uncertain key. After resolution, a retry of the old key returns `AI_REQUEST_FINAL`; a further explicit click can start a new operation. Changed input/new keys are still subject to admission. No self-service release or time-based automatic resend is provided.

Local development only: if provider completion cannot be established, stop all local workers using the DB, take a protected backup/ledger snapshot, and wait past the dispatch lease. Use `npm run ai:control -- resolve-local-unknown --url file:/absolute/local.db --request <request-uuid> --user <owner-uuid> --confirm-local-worker-stopped --acknowledge-unknown-provider-outcome --retain-maximum-cost`. This command requires `PATCH_ENV=development` and a file DB, matches the exact request and owner, and refuses active leases. It retains the maximum cost, operation identity and all other rows; it does not claim that OpenAI failed, finished or did not charge. Capture its `local_abandoned` diagnostic in an operator audit file. Verify only the target state changed, then restart the local worker. The old key stays terminal; only an explicit new operation may dispatch. Never use this development exception against production; production still requires the provider-final reconciliation above.

There are no separate mutable concurrency counters: admission derives occupancy and spend from ledger rows. Resolution releases only concurrency; retained costs continue through existing rolling budget windows. The `ai_operations` started marker is an idempotency tombstone, not an active reservation counter. It is intentionally retained. Unknown can still block indefinitely pending administrator action; automated provider reconciliation and a durable diagnostic store would be separate work.
...
`logAiDiagnostic` adds a separately allowlisted metadata record: server timestamp, internal request UUID, per-attempt correlation UUID, one-way key hash (never raw key), blocker UUID, endpoint, transition reason, duration/cost, normalized timeout/network/provider/parse/validation/persistence category, HTTP status, and provider `x-request-id`/recognized error code when available. Replays and rejected admissions are recorded without creating ledger rows. Errors/messages, response/prompt/source bodies, auth data and user identities are never serialized. Unrecognized provider codes/IDs are omitted. Configure protected stdout retention to preserve these records; no schema or migration is added, and past missing diagnostics cannot be reconstructed.
