...
## Integration merge validation — 2026-09-13

Integrated with `dc8f3f2` against Dev `976916b`; older branch-state statements above describe the original checkpoint. The cancel API now shares observeRoute. UI error parsing preserves the original ReliabilityError, including review conflict codes, rather than dropping kind/status/cooldown in a local wrapper. The explicitly requested Deep Link intent is the bounded read-resumption exception: retry only transient GET failures, respect Retry-After and the existing expiry, stop on access/auth/conflict/cancellation. Generic transport still dispatches once and never retries writes/AI. Combined transport tests verify retry identity after a truncated body plus account-bound cancellation through the bounded transport. All 209 tests, all browser/HTTP suites, Web/mobile/Swift and unsigned Simulator App+Widget builds passed. No provider or production changes.
