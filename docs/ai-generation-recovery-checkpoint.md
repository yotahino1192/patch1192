...
269 unit/API tests; typecheck; lint (zero errors, 13 existing warnings); Web build/artifact seal; local mobile build/artifact seal. Browser coverage: Add Material retry identity and classified errors (including explicit recovery), Review/Ready/save, Privacy consent and account isolation, and the existing Patch core integration flow. Mobile widths 320/390/430/768 tested; unresolved error screenshot inspected at 390px. Existing mobile bundle-size and Privacy fixture font allowlist warnings remain. Real provider generation/charging was deliberately not used as a test oracle.

## Follow-up: multiple-choice provider contract

A later real request reached OpenAI successfully (`providerStatus: 200`) but was rejected locally as `invalid_choices`: the provider repeated the correct answer separately from the choices and the two strings did not match exactly. The request was terminal and did not hold concurrency. The multiple-choice structured-output contract now asks for exactly four choices plus a bounded `correctChoiceIndex`; Patch derives `answer` from that indexed choice and removes the provider-only index before the material enters the existing domain. This removes the inconsistent duplicate representation while preserving the product/API card shape. QA and self-explain cards continue to return `answer` directly. No schema migration or retry behavior changed.
