...
Verified on Node 22.23.2: `npm run check` (181/181 unit tests, TypeScript, lint with 0 errors / 41 warnings, production web build), auth/privacy/onboarding+Retention browser suites, encrypted restore/Undo regression, Swift Widget/notification policy checks, `mobile:build:local`, Capacitor sync, unsigned iOS Simulator App + Widget build. A temporary Simulator installed/launched Patch and accepted `patch://continue`; it was removed afterward. Physical-device App Group sharing, OS delivery/widget display and signed release acceptance remain pending personal Apple Developer setup. Regression additionally verifies actual study/undo database results flowing into native notification and Widget output, account-scoped cleanup, and a signed-out Deep Link followed by email-form login and logout. Auth SDK and native delivery are mocked in these browser tests; production Clerk and physical OS delivery remain manual acceptance items. No external configuration, hosted migration or deployment was performed.
...
- Integrated AI Hardening with Privacy/Lifecycle Dev: gated admission/replay/dispatch/result, deletion-safe replay cleanup with budget preservation, unapplied AI migration renumbered to 0009, and combined regression coverage.
