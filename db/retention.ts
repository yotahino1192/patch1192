import { fingerprint, persistedContent, readStudySession } from './study.ts';
...
export async function reconcileStudy(tx:Transaction,userId:string,sessionId:string|null,now:string) {
 ...
 const session=(await tx.execute({sql:"SELECT * FROM study_sessions WHERE user_id=? AND id=?",args:[userId,sessionId]})).rows[0];if(!session || session.patch_id)return;
 ...
 if(complete&&!session.completed_at){const clock=await retentionClock(tx,userId,Date.parse(now));await tx.execute({sql:"UPDATE study_sessions SET completed_at=?,earned_day=?,qualifies=1 WHERE user_id=? AND id=?",args:[now,clock.day,userId,sessionId]});}
...
export async function startStudySession(userId:string,id:string,cardIds:string[],zone?:string) {
 await initializeDatabase();return database().transaction(async tx=>{
  ...
  const startHash=fingerprint(cardIds);
  const old=(await tx.execute({sql:"SELECT * FROM study_sessions WHERE user_id=? AND id=?",args:[userId,id]})).rows[0];
  if(old && (old.patch_id || old.onboarding || (old.start_hash ? old.start_hash!==startHash : (JSON.parse(String(old.card_ids)) as string[]).some((value,i)=>cardIds[i]!==value))))throw new InputError("セッションIDが別の操作に使われています。",409);
  if(old) {
    // Resuming an old unfinished session explicitly adopts processed-item completion.
    await tx.execute({sql:"UPDATE study_sessions SET qualifies=1 WHERE user_id=? AND id=? AND completed_at IS NULL",args:[userId,id]});
  ...
  await tx.execute({sql:"INSERT INTO study_sessions (user_id,id,set_id,card_ids,estimated_seconds,qualifies,created_at,start_hash) VALUES (?,?,?,?,?,1,?,?)",args:[userId,id,plan.cards[0].setId,JSON.stringify(selected),plan.estimatedSeconds,new Date().toISOString(),startHash]});
  return {id,cardIds:selected,estimatedSeconds:plan.estimatedSeconds,qualifies:true,session:await readStudySession(tx,userId,id)};
 });
}
