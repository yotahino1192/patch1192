...
import {PrivacyProvider} from '../../app/privacy-provider';
import {LessonEntry} from '../../features/my-lesson/lesson-entry';
import {apiFetch} from '../../lib/api-client';
import '../../app/globals.css';
const config=await (await fetch('/__lesson_identity')).json();
const fixture={openLesson:()=>{const url=new URL(location.href);url.searchParams.set('lesson',config.lessonId);history.pushState(null,'',url);window.dispatchEvent(new PopStateEvent('popstate'));},scope:null, switchAccount:null, logout:null, fail:false, failComplete:false, lose:false, slow:false, posts:0, aborts:0, helpCalls:0, helpDelay:0};window.lessonFixture=fixture;
const transport=async(path,options)=>{
 ...
 if(path==='/api/ai/chat') { fixture.helpCalls++; await new Promise(r=>setTimeout(r,fixture.helpDelay)); return Response.json({answer:'Test provider explanation for this Lesson'}); }
...
function Preview(){
 ...
 return <AccountContext.Provider value={{scope,logout:async()=>fixture.logout()}}><PrivacyProvider><LessonEntry><p>Test Home</p></LessonEntry></PrivacyProvider></AccountContext.Provider>;
}
