...
function Fixture(){const [screen,setScreen]=useState(state==='complete'?'study':'home');const session={...EMPTY_SESSION,id:'fixture-session',done:true,setId:'s0',total:6,mistakes:2};
const home=<Home data={data} now={now} startStudy={()=>window.uiFixture.starts++} setScreen={setScreen} selectSet={()=>{}} onContinue={()=>window.uiFixture.starts++} resumableSessions={state==='resume'?paused:[]} onResume={()=>window.uiFixture.starts++} onSample={async()=>{window.uiFixture.samples++;}}/>;
return <AccountContext.Provider value={state==='resume'?account:null}><Shell screen={screen} setScreen={setScreen}>{screen==='study'?<Study session={session} updateSession={()=>{}} data={data} queue={[]} flipped={false} setFlipped={()=>{}} setQueue={()=>{}} sessionDone={true} setSessionDone={()=>{}} sessionSetId="s0" sessionId={session.id} sessionTotal={6} sessionMistakes={2} setSessionMistakes={()=>{}} startStudy={()=>{}} setData={()=>{}} backToSets={()=>{}} goHome={()=>setScreen('home')} onPause={()=>{}} now={now}/>:home}</Shell></AccountContext.Provider>}
