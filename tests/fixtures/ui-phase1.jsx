...
function MainScreensFixture() {
  ...
  const [importDraft, setImportDraft] = useState({inputKind:params.has('topic')?'topic':'source',text:params.has('step')?'Memory and recall. 記憶を思い出す練習。'.repeat(5):'',attachments:[],detail:'標準',style:'一問一答',build:{step:/^[123]$/.test(step)?Number(step):step,coverage:params.has('whole')?'whole':'focus',focus:'Memory / 記憶',generation:{status:state==='error'?'failed':'running',error:'Connection interrupted. Your material is saved. Try again.'}}});
