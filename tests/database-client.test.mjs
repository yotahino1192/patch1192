...
test('fresh libSQL database initializes all schema and retains state on restart', async () => {
  ...
  try {
    ...
    assert.equal((await db.prepare('SELECT count(*) AS n FROM _loop_migrations').first()).n,8);
...
test('an existing Cloudflare schema is adopted without losing records', async () => {
  const client = createClient({url:':memory:'});
  try {
