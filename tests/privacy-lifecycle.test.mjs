...
globalThis.fetch=async(url,options)=>{assert.equal(url,'https://api.openai.com/v1/responses');calls++;return send?send(url,options):Response.json({output:[{content:[{type:'output_text',text:JSON.stringify({title:'T',category:'C',summary:'S',keyPoints:[],cards:[{question:'Q',answer:'A',format:'qa',choices:[],difficulty:1}]})}]}]});};
...
const req=(u,body,path='/api/data',claims)=>new Request(origin+path,{method:body?'POST':'GET',headers:headers(u.subject,u.userId,{'content-type':'application/json',...(claims?{authorization:'Bearer '+token(u.subject,claims)}:{})}),...(body?{body:JSON.stringify(body)}:{})});
...
test('duplicate concurrent operation and external timeout never automatically resend',async()=>{
 ...
 const responses=await Promise.all([GEN(req(u,b)),GEN(req(u,b))]);assert.deepEqual(responses.map(r=>r.status).sort(),[200,409]);assert.equal(calls,before+1);
 send=async()=>{throw Error('timeout');};try{const x=generation();assert.equal((await GEN(req(u,x))).status,502);const n=calls;assert.equal((await GEN(req(u,x))).status,409);assert.equal(calls,n);}finally{send=null;}
});
test('chat gate and post-flight consent check prevent chat persistence',async()=>{
 ...
 await consent(u);send=async()=>{await consent(u,'revoked');return Response.json({output:[{content:[{type:'output_text',text:'Answer'}]}]});};
 ...
 await consent(u);send=async()=>Response.json({output:[{content:[{type:'output_text',text:'Answer'}]}]});try{assert.equal((await CHAT(req(u,{...b,operationId:crypto.randomUUID()}))).status,200);}finally{send=null;}
});
...
test('duplicate workers use leases; DB failure rolls back data deletion and stays pending',async()=>{
 ...
 await db.prepare("CREATE TRIGGER fail_delete BEFORE DELETE ON cards BEGIN SELECT RAISE(ABORT,'forced'); END").run();
...
test('deletion during AI response discards result; worker authentication fails closed',async()=>{
 const u=await user();await consent(u);send=async()=>{await deletion(u);return Response.json({output:[{content:[{type:'output_text',text:JSON.stringify(material)}]}]});};
...
test('chat rejects denied, revoked and outdated consent; summary grant and generation fence work',async()=>{
 ...
 send=async()=>{await db.prepare('UPDATE users SET generation=generation+1 WHERE id=?').bind(u.userId).run();return Response.json({output:[{content:[{type:'output_text',text:JSON.stringify(material)}]}]});};
