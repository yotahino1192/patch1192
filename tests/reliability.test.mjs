...
test('combined AI transport retains truncated-body retry identity and delivers account-bound cancellation',async()=>{
 const old=Object.getOwnPropertyDescriptor(globalThis,'sessionStorage');Object.defineProperty(globalThis,'sessionStorage',{configurable:true,value:memory()});
 try{
  const seen=[],controller=new AbortController();let phase='broken';
  const native=async(url,init)=>{
   seen.push({url,headers:init.headers,body:init.body});
   if(url.endsWith('/cancel'))return Response.json({accepted:true});
   if(phase==='broken')return new Response('{"answer":',{status:200});
   controller.abort();await delay(10);return Response.json({answer:'late private result'});
  };
  const wrapped=(url,init)=>reliableRequest(native,url,init,1000);
  const options={method:'POST',headers:{'X-Patch-Account':owner.userId,'X-Patch-Session':owner.sessionId},body:'{}'};
  const broken=await sendAi(wrapped,'/api/ai/chat',options);await assert.rejects(readApiResponse(broken),e=>e.kind==='invalid_response');
  phase='cancel';await assert.rejects(sendAi(wrapped,'/api/ai/chat',{...options,signal:controller.signal}),e=>e.kind==='cancelled');
  await delay(20);assert.equal(seen.length,3);
  assert.equal(seen[0].headers.get('Idempotency-Key'),seen[1].headers.get('Idempotency-Key'));
  const cancel=seen.find(r=>r.url.endsWith('/cancel'));assert.ok(cancel);
  assert.equal(JSON.parse(cancel.body).operationKey,seen[0].headers.get('Idempotency-Key'));
  assert.equal(cancel.headers.get('X-Patch-Account'),owner.userId);assert.equal(cancel.headers.get('X-Patch-Session'),owner.sessionId);
  assert.match(cancel.headers.get('X-Request-ID'),/^[a-f0-9-]{36}$/);
 }finally{if(old)Object.defineProperty(globalThis,'sessionStorage',old);else delete globalThis.sessionStorage;}
});
